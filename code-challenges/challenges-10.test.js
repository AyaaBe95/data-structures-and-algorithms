...
const getNames = (arr) => {
  // Solution code here...
  let names = arr.map(value => value.name.split('').reverse().join(''));
  return names;
};
...
const count = (target, input) => {
  ...
  let counter = 0;
  input.map(arr => {
    let items = arr.filter(value => value === target);
...
const totalSum = (input) => {
  // Solution code here...
  let sum = 0;
  input.map(arr => arr.map(value => sum += value));
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let data = input.map(arr => {
    return arr.map(value => {
      return value % 5 === 0 && typeof value === 'number' ? Math.pow(2, value) : null;
    });
  });
  data = data.map(arr => {
...
let findMaleAndFemale = (data) => {
  ...
  output = output.map(element => element.name);
